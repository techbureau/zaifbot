...
class AutoCancelClient:
    ...
    def cancel_by_time(self, order_id, currency_pair, wait_sec):
        auto_cancel = AutoCancelByTime(self._key, self._secret, order_id, currency_pair, wait_sec)
        auto_cancel.run()
        self._auto_cancels_orders.append(auto_cancel)

    def cancel_by_price(self, order_id, currency_pair, target_margin):
        auto_cancel = AutoCancelByPrice(self._key, self._secret, order_id, currency_pair, target_margin)
        auto_cancel.run()
        self._auto_cancels_orders.append(auto_cancel)
    ...
    def get_active_order(self):
        ...
        for auto_cancel_order in self._auto_cancels_orders:
            active_orders.append(auto_cancel_order.get_info())
        return active_orders

    def take_back_cancel(self, id):
        for cancel_order in self._auto_cancels_orders:
            if id == id(cancel_order):
                cancel_order._stop_event = True
                return
            

class AutoCancelOrder(Thread, metaclass=ABCMeta):
...
class AutoCancelByTime(AutoCancelOrder):
    ...
    def get_info(self):
        item = {
            ...
            'current_price': get_current_last_price(self._currency_pair),
...
class AutoCancelByPrice(AutoCancelOrder):
