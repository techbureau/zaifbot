...
from zaifbot.modules.dao.moving_average import OhlcPricesDao, MovingAverageDao
import pandas as pd
...
class OhlcPricesSetUp:
    ...
    def execute(self, start_time, end_time):
        ...
        return self._ohlc_prices.create_data(target_ohlc_prices_record)
...
class MovingAverageSetUp:
    ...
    def execute(self, ma_start_time, tl_start_time, end_time):
        ...
        if len(target_epoch_times.index) == 0:
            return True
        ohlc_prices_moving_average =\
            self._get_ohlc_prices_moving_average(end_time, tl_start_time)
        for i in self._get_moving_average(ohlc_prices_moving_average, target_epoch_times):
    ...
    def _get_moving_average(self, ohlc_prices_moving_average, target_epoch_times):
        moving_average = {}
        for i in range(self._length, len(ohlc_prices_moving_average.index)):
            if ohlc_prices_moving_average.loc[i]['time'] in target_epoch_times.index:
                sma = ohlc_prices_moving_average.loc[i - self._length + 1:i]['close'].mean(axis=0)
                ema = self._get_ema(ohlc_prices_moving_average.loc[i - self._length:i])
                moving_average = {'time': int(ohlc_prices_moving_average.loc[i]['time']),
                                  'sma': sma, 'ema': ema,
                                  'closed': ohlc_prices_moving_average.loc[i]['closed']}
                yield moving_average

    def _get_ohlc_prices_moving_average(self, end_time, start_time):
        moving_average = \
            [i.__dict__ for i in self._moving_average.get_records(end_time, start_time, False)]
        ohlc_prices = \
            [j.__dict__ for j in self._ohlc_prices.get_records(end_time, start_time, False)]
        ohlc_prices_df = \
            pd.DataFrame(ohlc_prices, columns=['time', 'currency_pair', 'period', 'close', 'closed'])
        moving_average_df = \
            pd.DataFrame(moving_average, columns=['time', 'sma', 'ema', 'length'])
        ohlc_prices_moving_average = \
            ohlc_prices_df.merge(moving_average_df, left_on='time', right_on='time', how='outer')
        return ohlc_prices_moving_average

    def _get_ema(self, ohlc_prices_moving_average):
        k = 2 / (self._length + 1)
        if pd.isnull(ohlc_prices_moving_average.iloc[-2]['ema']):
            last_ema = ohlc_prices_moving_average.iloc[:-1]['close'].mean(axis=0)
        else:
            last_ema = ohlc_prices_moving_average.iloc[-2]['ema']
        return ohlc_prices_moving_average.iloc[-1]['close'] * k + last_ema * (1 - k)
